...
def derivate_session_salt(session_secret, size = 4):
...
def AES_encrypt_and_digest(args, key : bytes, msg : bytes, session_salt : bytes | None = None, seq_num : SimpleNamespace | None = None):
    if not args.use_seq_num_replay_resistance:     # Use timestamps
        ...
        seq_num.value += 1
        seq_num_bytes = struct.pack("!Q", seq_num.value)
        
        nonce = session_salt + seq_num_bytes
...
def AES_decrypt_and_verify(args, key : bytes, enc_data : bytes, session_salt : bytes | None = None, expected_seq_num : SimpleNamespace | None = None):
    if not args.use_seq_num_replay_resistance:     # Use timestamps
        ...
        if not args.disable_replay_resistance:  # Check if replay resistance is disabled
            if seq_num_msg <= expected_seq_num.value:
                print("!!! Sequence number is different !!!")
                raise ValueError
            expected_seq_num.value = seq_num_msg

        nonce = session_salt + seq_num_bytes_msg
